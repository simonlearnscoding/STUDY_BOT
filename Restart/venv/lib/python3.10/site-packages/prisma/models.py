...
class Switches(bases.BaseSwitches):
    """Represents a Switches record"""
    ...
    def create_partial(
        name: str,
        include: Optional[Iterable['types.SwitchesKeys']] = None,
        exclude: Optional[Iterable['types.SwitchesKeys']] = None,
        required: Optional[Iterable['types.SwitchesKeys']] = None,
        optional: Optional[Iterable['types.SwitchesKeys']] = None,
        relations: Optional[Mapping['types.SwitchesRelationalFieldKeys', str]] = None,
        exclude_relational_fields: bool = False,
    ) -> None:
        if not os.environ.get('PRISMA_GENERATOR_INVOCATION'):
            raise RuntimeError(
        ...
        fields: Dict['types.SwitchesKeys', PartialModelField] = OrderedDict()

        try:
            if include:
                for field in include:
                    fields[field] = _Switches_fields[field].copy()
            elif exclude:
                for field in exclude:
                    if field not in _Switches_fields:
                        raise KeyError(field)

                fields = {
                    key: data.copy()
                    for key, data in _Switches_fields.items()
                    if key not in exclude
                }
            else:
                fields = {
            ...
            if relations:
                raise ValueError('Model: "Switches" has no relational fields.')
        except KeyError as exc:
            raise ValueError(
                f'{exc.args[0]} is not a valid Switches / {name} field.'
            ) from None
...
_Switches_relational_fields: Set[str] = set()  # pyright: ignore[reportUnusedVariable]
_Switches_fields: Dict['types.SwitchesKeys', PartialModelField] = OrderedDict(
    [
        ('id', {
            'name': 'id',
            'is_list': False,
