...
class challenge(commands.Cog):
    ...
    async def MakeMessage(self, channel):
        Embed = discord.Embed()
        Embed.add_field(
            name=f"Day {time.localtime().tm_mday}", value=f"...", inline=False
        )
        if channel == vc.challenge_1:
            Embed.add_field(name="Done: ", value=f"{challenge.done1}\n", inline=False)
            Embed.add_field(
                name="Missing: ", value=f"{challenge.missing1}\n", inline=False
            )
            Embed.add_field(
                name="react with :raised_hand: if you did it", value="...", inline=False
            )
            ...
            Embed.add_field(
                name="Missing: ", value=f"{challenge.missing2}\n", inline=False
            )
            Embed.add_field(
                name="react with :raised_hand: if you did it", value="...", inline=False
            )
    ...
    async def sendMessageLost(self, userID, challengeId, reason):
        ...
        content = (
            f"Unfortunately, {member.mention}, you have lost the challenge {reason}"
        )
        try:
            await channel.send(content)
        except:
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.message_id == challenge.Message1.id:
            ...
            Embed.add_field(
                name=f"{payload.member}, Committing to the daily challenge! ",
                value=f"+ 10xp",
                inline=False,
            )
